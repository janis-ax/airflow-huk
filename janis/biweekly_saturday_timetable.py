...
Liegt bewusst im gleichen Ordner wie der DAG (`janis/`), damit Airflow beim Parsen
des DAGs das Modul findet. Der Ordner des DAG-Files wird temporaer auf sys.path gelegt;
ein Import aus dem git-gesyncten `plugins/`-Ordner ist im Cluster nicht zuverlaessig.

Warum nicht einfach "alle 14 Tage"?
----------------------------------
Eine starre 14-Tage-Kadenz ("biweekly") und "ungerade Kalenderwoche" sind NICHT
identisch. ISO-Jahre haben 52 ODER 53 Wochen. Hat ein Jahr 53 Wochen (ungerade),
folgt auf Woche 53 (ungerade) direkt Woche 1 (ungerade) des Folgejahres - das sind
zwei ungerade Wochen im Abstand von nur 7 Tagen. Eine reine +14-Tage-Logik wuerde
ab dort dauerhaft auf den geraden Wochen landen. Deshalb rechnen wir hier explizit
mit der ISO-Wochennummer statt mit einem festen Tagesabstand.
"""
...
SATURDAY = 5
# Nominale Fensterbreite fuer den allerersten Run (siehe Kommentar unten).
INTERVAL_DAYS = 14
